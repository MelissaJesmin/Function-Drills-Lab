...
let nameGreeting = nameCheck("Melissa")
...
//CODE HERE
const faveColorFinder = (color) => color === "red" ? 'red is a great color' : color === "green" ? 'green is a solid favorite color' : color === "black" ? 'so trendy' : 'you need to evaluate your favorite color choice'
...
//CODE HERE
const printAllNames = function(name_arr) {
...
//CODE HERE
function theEliminator(contestants_arr, loser_str) {
  for(let i = 0; i < contestants_arr.length; i++) {
    if(contestants_arr[i] === loser_str) {
      contestants_arr.splice(i,1)
    }
  }
...
//CODE HERE
function takes_string(str) {
  console.log(str.toUpperCase())
}

takes_string(sampleString)
...
function chocoFrogs(wallet) {
  ...
  return wallet/chocolatefrogcost
  
}
let totalFrogs = console.log(chocoFrogs(50))
...
function chocoFrogs2(gold) {
  let chocolatefrogcost = 3
  return Math.floor(gold/chocolatefrogcost)
}
let totalFrogs2 = console.log(chocoFrogs2(50))
...
//CODE HERE
function ascending(numArr) {
  //store the first element of the array to compare with arr[i]
  // let storeArr = numArr.slice(0)

  // //start from array index 1 because first index is already stored for comparision
  // for(let i = 1; i < numArr.length; i++) {
  //   //compare the index 1 with index 0, is index 1 is not smaller than 0 then store 1 in storeArr and repeat
  //   if(numArr[i] <= storeArr ) {
  //     return false
  //   }
  //   else {
  //     storeArr = numArr[i]
